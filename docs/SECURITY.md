...
| Saisie aberrante (calibrage) | 2 | **En place** (tranche 2, étape 3) : `MillimeterInput` n'accepte que des chiffres ASCII, un séparateur décimal et une plage 0,1–10 000 mm (ni « 1e3 », ni négatif, ni infini) ; `ScaleCalibration` borne ensuite le facteur à ±20 % ; l'export refuse une échelle invalide (`STLError.invalidScale`). Tests : `ScaleCalibrationTests.swift` |
